    ...
    public partial class MainWindow : Window
    {
        ...
        private void MuteTask_Click(object sender, RoutedEventArgs e)
        {
            if (sender is MenuItem menuItem && menuItem.Tag is string taskId)
            {
                ...
                {
                    task.IsMuted = true;
                    _controller.UpdateTask(task);  // Cập nhật task sau khi mute
        ...
        private void ShowTaskNotification(TaskItem task)
        {
            ...
            // Nếu bạn vẫn muốn phát âm thanh (nhưng không có âm thanh mặc định của MessageBox),
            // bạn có thể phát âm thanh như sau:
            try
            {
                var soundPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Assets", "Calendarsound.wav");
                var player = new System.Media.SoundPlayer(soundPath);
                player.Play();
            }
            catch (Exception ex)
            {
                // In ra lỗi nếu có sự cố khi phát âm thanh
